...
export class PortalAutoSelector {
    ...
    constructor(portalConfigs: PortalConfig[]) {
        this.portalConfigs = portalConfigs.filter(config => config.enabled && config.status === 'ATIVO');
    }
    ...
    selectPortals(criteria: PortalSelectionCriteria): AutoSelectionResult {
        ...
        return {
            ...
            scores,
            recommendations,
            totalEstimatedReach: scores.reduce((sum, score) => sum + score.estimatedReach, 0),
            totalEstimatedCost: scores.reduce((sum, score) => sum + score.estimatedCost, 0)
        };
    }
     ...
     */
    private calculatePortalScores(criteria: PortalSelectionCriteria): PortalScore[] {
        return this.portalConfigs.map(config => {
     ...
     */
    private calculateIndividualScore(config: PortalConfig, criteria: PortalSelectionCriteria): number {
        let score = 50; // Base score

        // Score baseado no tipo de imóvel
        const propertyTypeScore = this.getPropertyTypeScore(config, criteria.propertyType);
        score += propertyTypeScore * 0.3;

        // Score baseado no tipo de transação
        const transactionTypeScore = this.getTransactionTypeScore(config, criteria.transactionType);
     ...
     */
    private getPropertyTypeScore(config: PortalConfig, propertyType: PropertyData['propertyType']): number {
        const typeScores: Record<PortalType, Record<PropertyData['propertyType'], number>> = {
            ZAP: {
                apartment: 95,
                house: 85,
                commercial: 90,
     ...
     */
    private getTransactionTypeScore(config: PortalConfig, transactionType: PropertyData['transactionType']): number {
        const transactionScores: Record<PortalType, Record<PropertyData['transactionType'], number>> = {
            ZAP: {
                sale: 95,
                rent: 90
            },
    ...
    private getPriceScore(config: PortalConfig, price: number): number {
        const priceRanges: Record<PortalType, { min: number; max: number; score: number }[]> = {
            ZAP: [
        ...
        const applicableRange = ranges.find(range => price >= range.min && price <= range.max);
     ...
     */
    private getLocationScore(config: PortalConfig, location: { city: string; state: string }): number {
        // Simulação de dados de cobertura por portal
        const coverageData: Record<PortalType, { states: string[]; score: number }> = {
            ...
            MERCADO_LIVRE: { states: ['SP', 'RJ', 'MG', 'PR', 'SC', 'RS', 'BA', 'PE'], score: 85 },
     ...
     */
    private getFeaturesScore(config: PortalConfig, features?: PropertyData['features']): number {
     ...
     */
    private estimateReach(config: PortalConfig, criteria: PortalSelectionCriteria): number {
        ...
        const propertyTypeMultiplier = this.getPropertyTypeMultiplier(config.type, criteria.propertyType);

        // Ajuste baseado no tipo de transação
        const transactionTypeMultiplier = this.getTransactionTypeMultiplier(config.type, criteria.transactionType);

        return Math.round(reach * locationMultiplier * propertyTypeMultiplier * transactionTypeMultiplier);
    }

    /**
     * Estima o custo de publicação em um portal
     */
    private estimateCost(config: PortalConfig, criteria: PortalSelectionCriteria): number {
     ...
     */
    private selectBestPortals(scores: PortalScore[], criteria: PortalSelectionCriteria): PortalId[] {
     ...
     */
    private getScoreReasons(config: PortalConfig, criteria: PortalSelectionCriteria): string[] {
        ...
        if (this.getLocationScore(config, criteria.location) > 80) {
            reasons.push(`Forte cobertura em ${criteria.location.city}/${criteria.location.state}`);
        }
     ...
     * Gera recomendações
     */
    private generateRecommendations(scores: PortalScore[], criteria: PortalSelectionCriteria): string[] {
        ...
        const totalReach = scores.reduce((sum, score) => sum + score.estimatedReach, 0);
        if (totalReach > 10000000) {
            recommendations.push('Alcance total estimado superior a 10 milhões');
        }

        const totalCost = scores.reduce((sum, score) => sum + score.estimatedCost, 0);
        if (totalCost > 500) {
     ...
     */
    private getLocationMultiplier(location: { city: string; state: string }): number {
        // Cidades principais
        const mainCities = ['São Paulo', 'Rio de Janeiro', 'Belo Horizonte', 'Porto Alegre', 'Curitiba'];
        if (mainCities.includes(location.city)) return 1.2;

        // Capitais
        const capitals = ['São Paulo', 'Rio de Janeiro', 'Belo Horizonte', 'Porto Alegre', 'Curitiba', 'Florianópolis', 'Brasília'];
     ...
     */
    private getPropertyTypeMultiplier(portalType: PortalType, propertyType: PropertyData['propertyType']): number {
        const multipliers: Record<PortalType, Record<PropertyData['propertyType'], number>> = {
            ZAP: {
                apartment: 1.2,
                house: 1.1,
                commercial: 1.0,
     ...
     */
    private getTransactionTypeMultiplier(portalType: PortalType, transactionType: PropertyData['transactionType']): number {
        const multipliers: Record<PortalType, Record<PropertyData['transactionType'], number>> = {
            ZAP: {
                sale: 1.1,
                rent: 1.0
            },
