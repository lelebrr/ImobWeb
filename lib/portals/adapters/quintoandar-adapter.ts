...
export class QuintoAndarAdapter extends BasePortalAdapter implements PortalAdapter {
    ...
    constructor(config: Record<string, any>) {
        ...
        this.baseUrl = config.baseUrl || 'https://api.quintoandar.com.br';
    }
    ...
    async createProperty(data: Record<string, unknown>): Promise<string> {
        ...
        try {
            const response = await this.makeApiCall('/properties', {
                method: 'POST',
                body: xml
            });
            ...
            throw new Error(`Failed to create property on QuintoAndar: ${error.message}`);
        }
    }
    ...
    async updateProperty(externalId: string, data: Record<string, unknown>): Promise<void> {
        ...
        try {
            await this.makeApiCall(`/properties/${externalId}`, {
                method: 'PUT',
                body: xml
            });
        } catch (error) {
            throw new Error(`Failed to update property on QuintoAndar: ${error.message}`);
        }
    }
    ...
    async deleteProperty(externalId: string): Promise<void> {
        try {
            ...
            throw new Error(`Failed to delete property on QuintoAndar: ${error.message}`);
        }
    }

    async getProperty(externalId: string): Promise<Record<string, unknown>> {
        try {
            const response = await this.makeApiCall(`/properties/${externalId}`);
            return this.parseXmlResponse(response);
        } catch (error) {
            throw new Error(`Failed to get property from QuintoAndar: ${error.message}`);
        }
    }
    ...
    async getAnalytics(propertyId?: string): Promise<Record<string, unknown>> {
        try {
            const endpoint = propertyId ? `/properties/${propertyId}/analytics` : '/analytics';
            const response = await this.makeApiCall(endpoint);
            return response;
        } catch (error) {
            throw new Error(`Failed to get analytics from QuintoAndar: ${error.message}`);
        }
    }
    ...
    validateProperty(property: PropertyData): { valid: boolean; errors?: string[] } {
        ...
        if (property.photos && property.photos.length > 30) {
            errors.push('Maximum 30 photos allowed');
        }
        ...
        if (property.features?.area && property.features.area < 15) {
            errors.push('Minimum area is 15m²');
        }
  ...
  <metadata>
    ...
    <status><![CDATA[${property.status}]]></status>
  </metadata>
</property>`;
...
export function createQuintoAndarAdapter(config: Record<string, any>): QuintoAndarAdapter {
    return new QuintoAndarAdapter(config);
}