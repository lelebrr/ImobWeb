 ...
 */
export class VivaRealAdapter extends BasePortalAdapter implements PortalAdapter {
    ...
    validateProperty(property: PropertyData): PropertyValidation {
        ...
        if (!property.address?.neighborhood) {
            warnings.push('Recomendado informar o bairro para melhor localização no Viva Real');
        }
    ...
    async createProperty(data: Record<string, unknown>): Promise<string> {
        try {
            ...
            const xmlPayload = xmlGenerator.generate(data as unknown as PropertyData, 'viva');
            ...
            if (!match) {
                throw new Error('Failed to extract property ID from Viva Real response');
            }

            return match[1];
        } catch (error) {
            console.error('[VivaRealAdapter] Error creating property:', error);
            throw new Error(`Failed to create property on Viva Real: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
    ...
    async updateProperty(externalId: string, data: Record<string, unknown>): Promise<void> {
        try {
            ...
            throw new Error(`Failed to update property on Viva Real: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
    ...
    async deleteProperty(externalId: string): Promise<void> {
        try {
            ...
            throw new Error(`Failed to delete property on Viva Real: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
    ...
    async getProperty(externalId: string): Promise<Record<string, unknown>> {
        try {
            ...
            throw new Error(`Failed to get property from Viva Real: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
    ...
    async getLeads(): Promise<any[]> {
        try {
            ...
            throw new Error(`Failed to get leads from Viva Real: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
    ...
    async getAnalytics(propertyId?: string): Promise<Record<string, unknown>> {
        try {
            ...
            throw new Error(`Failed to get analytics from Viva Real: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
    ...
    async activateBadge(
        ...
        try {
            await this.makeRequest(`${this.getEndpoint()}/imoveis/${externalId}/badge`, {
                method: 'POST',
                headers: {
            ...
            throw new Error(`Failed to activate badge on Viva Real: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
 ...
 */
export function createVivaRealAdapter(config: Record<string, any>): PortalAdapter {
