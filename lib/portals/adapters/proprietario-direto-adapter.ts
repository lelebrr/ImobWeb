  ...
  implements PortalAdapter
{
  ...
  async getAnalytics(propertyId?: string): Promise<{
    ...
    try {
      ...
      throw new Error(
        `Failed to get analytics from Proprietário Direto: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
    }
  }
...
export function createProprietarioDiretoAdapter(
  config: Record<string, any>,
): ProprietarioDiretoAdapter {
