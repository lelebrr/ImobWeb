...
export class OlxAdapter extends BasePortalAdapter implements PortalAdapter {
    ...
    async createProperty(data: Record<string, unknown>): Promise<string> {
        try {
            ...
            const xmlPayload = xmlGenerator.generate(data as unknown as PropertyData, 'olx');
            ...
            throw new Error(`Failed to create property on OLX: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
    ...
    async updateProperty(externalId: string, data: Record<string, unknown>): Promise<void> {
        try {
            ...
            throw new Error(`Failed to update property on OLX: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
    ...
    async deleteProperty(externalId: string): Promise<void> {
        try {
            ...
            throw new Error(`Failed to delete property on OLX: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
    ...
    async getProperty(externalId: string): Promise<Record<string, unknown>> {
        try {
            const response = await this.makeRequest(`${this.getEndpoint()}/ads/${externalId}`, {
                headers: this.getAuthHeaders(),
            });
            ...
            throw new Error(`Failed to get property from OLX: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
    ...
    async getLeads(): Promise<any[]> {
        try {
            ...
            throw new Error(`Failed to get leads from OLX: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
    ...
    async getAnalytics(propertyId?: string): Promise<Record<string, unknown>> {
        try {
            ...
            throw new Error(`Failed to get analytics from OLX: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
    ...
    async activateChat(externalId: string): Promise<void> {
        try {
            ...
            throw new Error(`Failed to activate chat on OLX: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
