 ...
 */
export class ChavesNaMaoAdapter extends BasePortalAdapter implements PortalAdapter {
    ...
    validateProperty(property: PropertyData): PropertyValidation {
        ...
        if (property.price && property.price > 800000 && !property.highlightPackage) {
    ...
    async createProperty(data: Record<string, unknown>): Promise<string> {
        try {
            ...
            const xmlPayload = xmlGenerator.generate(data as unknown as PropertyData, 'chaves');
            ...
            if (!match) {
                throw new Error('Failed to extract property ID from Chaves na Mão response');
            }

            return match[1];
        } catch (error) {
            console.error('[ChavesNaMaoAdapter] Error creating property:', error);
            throw new Error(`Failed to create property on Chaves na Mão: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
    ...
    async updateProperty(externalId: string, data: Record<string, unknown>): Promise<void> {
        try {
            ...
            throw new Error(`Failed to update property on Chaves na Mão: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
    ...
    async deleteProperty(externalId: string): Promise<void> {
        try {
            ...
            throw new Error(`Failed to delete property on Chaves na Mão: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
    ...
    async getProperty(externalId: string): Promise<Record<string, unknown>> {
        try {
            ...
            throw new Error(`Failed to get property from Chaves na Mão: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
    ...
    async getLeads(): Promise<any[]> {
        try {
            ...
            throw new Error(`Failed to get leads from Chaves na Mão: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
    ...
    async getAnalytics(propertyId?: string): Promise<Record<string, unknown>> {
        try {
            ...
            throw new Error(`Failed to get analytics from Chaves na Mão: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
    ...
    async activateHighlight(
        ...
        try {
            ...
            throw new Error(`Failed to activate highlight on Chaves na Mão: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
 ...
 */
export function createChavesNaMaoAdapter(config: Record<string, any>): PortalAdapter {
