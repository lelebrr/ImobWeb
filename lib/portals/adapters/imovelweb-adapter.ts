 ...
 */
export class ImovelWebAdapter extends BasePortalAdapter implements PortalAdapter {
    ...
    validateProperty(property: PropertyData): PropertyValidation {
        ...
        if (property.area && property.area > 150 && !property.virtualTour) {
            suggestions.push('Para imóveis grandes, considere adicionar um tour virtual');
        }
    ...
    async createProperty(data: Record<string, unknown>): Promise<string> {
        try {
            ...
            const xmlPayload = xmlGenerator.generate(data as unknown as PropertyData, 'imovelweb');
            ...
            if (!match) {
                throw new Error('Failed to extract property ID from ImovelWeb response');
            }

            return match[1];
        } catch (error) {
            console.error('[ImovelWebAdapter] Error creating property:', error);
            throw new Error(`Failed to create property on ImovelWeb: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
    ...
    async updateProperty(externalId: string, data: Record<string, unknown>): Promise<void> {
        try {
            ...
            throw new Error(`Failed to update property on ImovelWeb: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
    ...
    async deleteProperty(externalId: string): Promise<void> {
        try {
            ...
            throw new Error(`Failed to delete property on ImovelWeb: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
    ...
    async getProperty(externalId: string): Promise<Record<string, unknown>> {
        try {
            ...
            throw new Error(`Failed to get property from ImovelWeb: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
    ...
    async getLeads(): Promise<any[]> {
        try {
            ...
            throw new Error(`Failed to get leads from ImovelWeb: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
    ...
    async getAnalytics(propertyId?: string): Promise<Record<string, unknown>> {
        try {
            ...
            throw new Error(`Failed to get analytics from ImovelWeb: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
    ...
    async activateHighlight(
        ...
        try {
            ...
            throw new Error(`Failed to activate highlight on ImovelWeb: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
 ...
 */
export function createImovelWebAdapter(config: Record<string, any>): PortalAdapter {
