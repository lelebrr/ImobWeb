...
export abstract class BasePortalAdapter implements PortalAdapter {
  ...
  protected validatePropertyBase(property: PropertyData): PropertyValidation {
    ...
    if (property.title && property.title.length > this.getMaxTitleLength()) {
      errors.push(
        `O título não pode ultrapassar ${this.getMaxTitleLength()} caracteres`,
      );
    }
    if (
      property.description &&
      property.description.length > this.getMaxDescriptionLength()
    ) {
      errors.push(
        `A descrição não pode ultrapassar ${this.getMaxDescriptionLength()} caracteres`,
      );
    }
    ...
    const hasPhotos =
      property.photos && property.photos.length >= this.getMinPhotos();
  ...
  abstract getAnalytics(propertyId?: string): Promise<{
