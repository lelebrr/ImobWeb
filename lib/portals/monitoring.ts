...
export class IntegrationMonitor {
     ...
     */
    async logEvent(event: Omit<IntegrationEvent, 'id'>): Promise<string> {
        try {
     ...
     */
    async getHealthMetrics(portalId: PortalId): Promise<IntegrationHealthMetrics> {
        const now = new Date();
        ...
        const totalResponseTime = events.reduce((sum, e) => sum + (e.responseTime || 0), 0);
    ...
    private async checkIntegrationHealth(portalId: PortalId): Promise<void> {
        try {
            ...
            if (metrics.averageResponseTime > this.config.alertThresholds.responseTime) {
                alerts.push(`High response time: ${metrics.averageResponseTime.toFixed(2)}ms`);
            }
            ...
            if (alerts.length > 0) {
                await this.sendAlert(portalId, 'Health check failed', alerts.join(', '));
            }
            ...
            console.error(`[IntegrationMonitor] Error checking health for ${portalId}:`, error);
     ...
     */
    private async sendAlert(portalId: PortalId, title: string, message: string): Promise<void> {
     ...
     */
    private async sendEmailAlert(message: string, event?: IntegrationEvent): Promise<void> {
     ...
     */
    private async sendWebhookAlert(message: string, event?: IntegrationEvent): Promise<void> {
        ...
        try {
            ...
            console.error('[IntegrationMonitor] Failed to send webhook alert:', error);
        }
    }
     ...
     */
    private async sendSlackAlert(message: string, event?: IntegrationEvent): Promise<void> {
                ...
                body: JSON.stringify({
                    ...
                    attachments: event ? [{
                        ...
                        fields: [
                            ...
                            { title: 'Timestamp', value: event.timestamp.toISOString(), short: false }
                        ]
                    }] : []
                })
            });
    ...
    private async generateDailyReport(): Promise<void> {
        ...
        try {
            ...
            const report = {
                ...
                averageResponseTime: events.reduce((sum, e) => sum + (e.responseTime || 0), 0) / events.length || 0,
            ...
            console.error('[IntegrationMonitor] Failed to generate daily report:', error);
        }
    }
...
export function startMonitoring(config?: MonitoringConfig): void {
    if (config) {
        integrationMonitor.config = config;
    }
 ...
 */
export async function logIntegrationEvent(event: Omit<IntegrationEvent, 'id'>): Promise<string> {
