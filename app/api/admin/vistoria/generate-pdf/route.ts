...
function buildPhotoHtml(photo: PhotoData, globalIdx: number, roomName: string): string {
  ...
  if (hasAnnotations) {
    ...
    svgContent = sorted.map((ann, i) => {
      const cx = (ann.x / 100) * svgW; const cy = (ann.y / 100) * svgH; const num = i + 1;
      ...
      return `
        ...
        <text x="${cx}" y="${cy + 5}" text-anchor="middle" fill="white" font-size="11" font-weight="800" font-family="Arial">${num}</text>
        ...
        <text x="${goRight ? labelX + 4 : labelX - 4}" y="${cy + 4}" text-anchor="${textAnchor}" fill="white" font-size="9.5" font-weight="700" font-family="Arial">${ann.label}</text>`;
    }).join('\n');
  }
...
function generateHtml(data: VistoriaData): string {
  ...
  const roomsHtml = data.rooms.map((room, idx) => {
    const photosHtml = room.photos && room.photos.length > 0
      ? `<div class="photo-grid">${room.photos.map((photo) => { globalPhotoIdx++; return buildPhotoHtml(photo, globalPhotoIdx, room.name); }).join('\n')}</div>`
      : '';
  ...
  const andarText = data.andar ? `<br>Andar: ${data.andar}` : '';
  const conjText = data.conjApto ? `<br>Conjunto: ${data.conjApto}` : '';
  const aptoText = data.apto ? `<br>Apartamento: ${data.apto}` : '';
...
<style>
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .cover::before { content: ''; position: absolute; top: 0; left: 0; right: 0; height: 6px; background: linear-gradient(90deg, #0b5bd3, #667eea, #8b5cf6); }
  .cover-badge { display: inline-flex; align-items: center; gap: 6px; background: #f0f9ff; border: 1px solid #bae6fd; color: #0369a1; padding: 6px 16px; border-radius: 20px; font-size: 8pt; font-weight: 700; text-transform: uppercase; letter-spacing: 1px; margin-bottom: 30px; }
  .cover-title { font-size: 32pt; font-weight: 900; color: #0F172A; letter-spacing: -1px; line-height: 1.1; }
  .cover-highlight { display: block; background: linear-gradient(135deg, #0b5bd3, #667eea); -webkit-background-clip: text; -webkit-text-fill-color: transparent; font-size: 36pt; margin-top: 8px; }
  .cover-subtitle { font-size: 14pt; color: #64748b; margin-top: 12px; font-weight: 400; }
  ...
  .cover-footer { position: absolute; bottom: 30px; font-size: 8pt; color: #cbd5e1; }

  /* === CRITERIA PAGE === */
  .criteria-page { page-break-after: always; }
  .criteria-title { font-size: 11pt; font-weight: 800; color: #0F172A; margin-bottom: 16px; padding-bottom: 8px; border-bottom: 2px solid #e2e8f0; }
  ...
  /* === LEGAL TEXT === */
  .legal-box { background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 8px; padding: 16px 18px; margin: 16px 0; }
  .legal-text { font-size: 9pt; color: #475569; line-height: 1.7; text-align: justify; margin-bottom: 8px; }

  /* === INFO GRID === */
  .info-section { page-break-after: always; }
  .section-title { font-size: 13pt; font-weight: 800; color: #0F172A; padding-bottom: 8px; border-bottom: 3px solid #0b5bd3; margin-bottom: 16px; }
  ...
  .summary-stat { padding: 10px 16px; background: #f0f9ff; border: 1px solid #bae6fd; border-radius: 8px; text-align: center; min-width: 100px; }
  .summary-stat .num { font-size: 20pt; font-weight: 900; color: #0b5bd3; }
  .summary-stat .label { font-size: 7.5pt; color: #64748b; text-transform: uppercase; font-weight: 600; letter-spacing: 0.5px; }

  /* === SUMARIO === */
  .sumario-page { page-break-after: always; }
  .sumario-title { font-size: 13pt; font-weight: 800; color: #0F172A; margin-bottom: 16px; }
  ...
  /* === ROOMS === */
  .rooms-title { font-size: 12pt; font-weight: 800; color: #0F172A; border-bottom: 3px solid #0b5bd3; padding-bottom: 8px; margin-bottom: 20px; }
  ...
  .considerations h2 { font-size: 13pt; font-weight: 800; color: #0F172A; border-bottom: 3px solid #0b5bd3; padding-bottom: 8px; margin-bottom: 16px; }
  ...
  .dispute-box { margin-top: 24px; padding: 16px 18px; background: #fefce8; border: 1px solid #fde68a; border-radius: 10px; }
  .dispute-box h3 { font-size: 11pt; color: #92400e; font-weight: 800; margin-bottom: 10px; }
  .dispute-box ol { margin-left: 18px; font-size: 9pt; color: #78350f; }
  .dispute-box li { margin-bottom: 5px; line-height: 1.5; }

  .contest-email { background: #f0f9ff; border: 1px solid #bae6fd; border-radius: 8px; padding: 12px 16px; margin-top: 16px; font-size: 9pt; color: #0369a1; }

  /* === SIGNATURES === */
  .signatures { page-break-before: always; padding-top: 30px; }
  .signatures p { font-size: 10pt; color: #475569; margin-bottom: 10px; }
  ...
  .final-note { margin-top: 40px; padding: 12px; background: #f8fafc; border-radius: 8px; font-size: 8pt; color: #94a3b8; text-align: center; border: 1px solid #f1f5f9; }
</style>
</head>
<body>
...
<div class="cover">
  ...
  <div class="cover-footer">imobWeb · Sistema de Vistoria com IA</div>
</div>
...
<div class="criteria-page">
  ...
  <div class="criteria-grid">
    ...
    <div class="criteria-item" style="grid-column: span 2; border-left-color: #dc2626;"><div class="criteria-label" style="color: #dc2626;">Estado ruim</div><div class="criteria-desc">Danos graves, podendo conter: infiltração, rachaduras, danificações, pragas e vazamento</div></div>
  </div>

  <div class="section-title">Termo de vistoria de entrada</div>
  <div class="legal-box">
    <p class="legal-text">O presente anexo faz parte integrante do "Contrato de Locação" do imóvel CONDOMÍNIO ${data.condominio} situado na ${data.endereco} nº ${data.numero}${conjText}${aptoText} - CEP: ${data.cep} – ${data.bairro} – ${data.cidade}/${data.estado}, datado de ${formatDateLong(data.dataLaudo)}, tendo como partes:</p>
    ...
    <p class="legal-text" style="margin-top: 12px;">Este laudo de vistoria tem por objetivo retratar o estado de conservação e funcionamento do imóvel na data de sua realização, em atendimento ao disposto no art. 2, inciso V e art. 23, inciso III, ambos da Lei nº 8.245/91 (Lei do Inquilinato). A presente Vistoria foi realizada por observância estética da construção e acabamentos, não se atendo a aspectos estruturais de solidez, fundações e vícios ocultos.</p>
  </div>
  ...
  <div class="info-grid">
    ...
    <div class="info-card"><div class="info-label">Fotos</div><div class="info-value">${totalPhotos} registro(s)</div></div>
  </div>
...
<div class="signatures">
  ...
  <p style="margin-top: 30px;">São Paulo, ${formatDateLong(data.dataLaudo)}.</p>
  ...
  <div class="final-note">Laudo gerado automaticamente por imobWeb · Vistoria com IA · ${formatDateLong(data.dataLaudo)}</div>
</div>
