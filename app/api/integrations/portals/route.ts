...
export async function GET(req: NextRequest) {
    try {
        ...
        const portalTypes: PortalType[] = ['ZAP', 'VIVAREAL', 'OLX', 'IMOVELWEB', 'CHAVES_NA_MAO', 'MERCADO_LIVRE', 'PROPRIETARIO_DIRETO', 'IMOBIBRASIL', 'LOFT', 'QUINTO_ANDAR']

        const portalData = await Promise.all(portalTypes.map(async (portalType) => {
            const integrationId = portalType.toLowerCase()
            const integration = integrations[integrationId]

            // Get adapter for health check
            let health = { status: 'unknown', lastCheck: null, error: null }
            let stats = { totalProperties: 0, activeProperties: 0, totalViews: 0, totalLeads: 0 }
            let syncStatus = { lastSync: null, nextSync: null, isSyncing: false }

            try {
                if (integration?.status === 'connected' && integration?.config) {
                    const adapter = getPortalAdapter(portalType, integration.config)
...
export async function POST(req: NextRequest) {
    try {
        ...
        if (!portalId || !action) {
            return NextResponse.json({ error: 'Portal ID and action are required' }, { status: 400 })
        }
        ...
        switch (action) {
            ...
            case 'disconnect':
                ...
                const { [portalId]: removed, ...remainingIntegrations } = currentIntegrations
            ...
            case 'test':
                // Test the connection
                try {
                    const adapter = getPortalAdapter(portalId as PortalType, config || {})
...
function getPortalFeatures(portalType: PortalType): string[] {
    const features: Record<PortalType, string[]> = {
        ...
        CUSTOM: ['Configuração personalizada', 'Campos customizados', 'Lógica personalizada']
    }
    return features[portalType] || []
}

function getPortalDocumentation(portalType: PortalType): { url: string; description: string } {
